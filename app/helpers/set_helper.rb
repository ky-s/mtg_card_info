module SetHelper
  ...
  def fetch(set)
    return link_to('Fetched', set_fetch_cards_path(set: set.code), remote: true, class: %w(btn btn-sm btn-secondary m-2)) if set.fetched?
  ...
  def fetch_image(set)
    ...
    return link_to('Fetched', set_fetch_cards_image_path(set: set.code), remote: true, class: %w(btn btn-sm btn-secondary m-2)) if set.image_fetched?
