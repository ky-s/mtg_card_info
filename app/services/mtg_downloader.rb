module MTGDownloader
  ...
  def fetch_card(set_code)
    ...
    cards.each.with_index(1) do |card, c|
      ...
      ActionCable.server.broadcast('progress:1', percent: (c.to_f / cards.size.to_f * 100).to_i, set: set_code, action: 'fetch')
    end
  ...
  def fetch_card_image(set_code)
    ...
    cards.each.with_index(1) do |mtg_card, c|
      ...
      ActionCable.server.broadcast('progress:1', percent: (c.to_f / cards.size.to_f * 100).to_i, set: set_code, action: 'fetch_image')
